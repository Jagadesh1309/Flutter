...
class TestUtils {
  ...
  Future<void> verifyElementExists(SerializableFinder finder,
      {Duration timeout = const Duration(seconds: 5),
      String? errorMessage}) async {
    print('Verifying existence of the element...');

    bool exists = await isElementExists(finder, timeout: timeout);
    if (exists) {
      print('Element found.');
    } else {
      print('Element not found.');
      throw Exception(
          errorMessage ?? 'Element not found within the specified timeout.');
    }
  }
  ...
  Future<void> scrollTo(SerializableFinder finder, double dx, double dy,
  ...
  Future<void> verifyTextExists(SerializableFinder finder, String expectedText,
    ...
    try {
      for (var index = 0; index < widgetLength; index++) {
        ...
        final finderToVerify = expectedText is List
            ...
            : '$finder$index';
        await verifyTextExists(find.byValueKey(finderToVerify), textToVerify,
  ...
  Future<void> captureScreenshot(String testName) async {
    try {
      await File('tests/test_report/screenshots/$testName.png')
